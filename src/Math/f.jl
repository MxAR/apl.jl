@everywhere module f
	...
	##===================================================================================
	## fibonacci lattice 2d
	##	N: number of points in the latice
	## 	i: index of the point 
	##===================================================================================
	export fib_lattice_2d

	##-----------------------------------------------------------------------------------
	function fib_lattice_2d(N::Z, i::Z) where Z<:Integer
		return [(i - .5)/N, rem(i / constants.fib_phi, 1)]	
	end
	...
	##-----------------------------------------------------------------------------------
	function arg(z::C) where C<:Complex
	...
	## C(n, k) * p^k * (1 - p)^(n - k)
	##-----------------------------------------------------------------------------------
	function binomp(n::Z, k::Z, p::R) where Z<:Integer where R<:AbstractFloat
		return binomp(k, n - k, p, R(1) - p)
	end
	...
	##-----------------------------------------------------------------------------------
	function cross(u::Array{T, 1}, v::Array{T, 1}) where T<:Number
